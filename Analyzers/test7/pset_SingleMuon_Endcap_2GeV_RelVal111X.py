...
# with command line options: L1TMuonSimulations/Configuration/python/SingleMuonFlatOneOverPt2To7000_PositiveEndCap_cfi.py --step GEN,SIM,DIGI:pdigi_valid,L1,L1TrackTrigger --conditions auto:phase2_realistic_T14 --eventcontent FEVTDEBUGHLT --datatier GEN-SIM-DIGI-RAW --era Phase2C8 --beamspot HLLHC14TeV --geometry Extended2026D41 --customise SimGeneral/MixingModule/customiseStoredTPConfig.higherPtTP,SLHCUpgradeSimulations/Configuration/aging.customise_aging_1000 --python_filename pset_SingleMuon_Endcap_2GeV_RelVal11x1y0z.py --fileout file:SingleMuon_Endcap.root --mc --processName L1 --no_exec --nThreads 4 -n 100
import FWCore.ParameterSet.Config as cms

from Configuration.Eras.Era_Phase2C8_cff import Phase2C8

process = cms.Process('L1',Phase2C8)
...
if True:
    ...
    process.doAllDigiTask = cms.Task(process.generatorSmeared, process.muonDigiTask)  # removed process.calDigiTask
    process.SimL1TMuonTask = cms.Task(process.SimL1TMuonCommonTask, process.me0TriggerAllDigiTask, process.rpcRecHits, process.simBmtfDigis, process.simEmtfDigis, process.simGmtCaloSumDigis, process.simGmtStage2Digis, process.simMuonGEMPadTask, process.simOmtfDigis, process.simTwinMuxDigis)  # added process.rpcRecHits
