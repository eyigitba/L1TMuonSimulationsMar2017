...
# https://github.com/tensorflow/tensorflow/blob/r2.4/tensorflow/python/keras/layers/preprocessing/normalization.py#L51-L374
# ==============================================================================
...
class Normalization(base_preprocessing_layer.CombinerPreprocessingLayer):
  ...
  def set_weights(self, weights):
    ...
    if len(weights) == 2:
      weights.append(np.zeros(weights[0].shape, dtype=np.int64))
...
class _NormalizingCombiner(base_preprocessing_layer.Combiner):
  ...
  def add_data_to_accumulator(self, count, mean, variance, accumulator):
    ...
    accumulator[self.COUNT_IDX] = np.nan_to_num(combined_count)
  ...
  def _create_accumulator(self, count, mean, variance):
    ...
    return [np.nan_to_num(count), np.nan_to_num(mean), np.nan_to_num(variance)]
